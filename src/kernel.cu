...
simpson_f1(float left, float width, float *tuple) {
  ...
  return (width / 6) * (g_f1(left, tuple) +
                        g_f1(right, tuple) +
                        g_f1(mid, tuple) * 4);
}
__device__ float
simpson_f2(float left, float width, float *tuple) {
  float mid = left + width * 0.5;
  float right = left + width;
  return (width / 6) * (g_f2(left, tuple) +
                        g_f2(right, tuple) +
                        g_f2(mid, tuple) * 4);
}
__device__ float
simpson_f3(float left, float width, float *tuple) {
  float mid = left + width * 0.5;
  float right = left + width;
  return (width / 6) * (g_f3(left, tuple) +
                        g_f3(right, tuple) +
                        g_f3(mid, tuple) * 4);
}
__device__ float
simpson_f12_multi(float left, float width, float *x) {
  float mid = left + width * 0.5;
  float right = left + width;
  return (width / 6) * (g_f12_multi(left, x) +
                        g_f12_multi(right, x) +
                        g_f12_multi(mid, x) * 4);
}
__device__ float
simpson_f3_multi(float left, float width, float *x, float *y) {
  float mid = left + width * 0.5;
  float right = left + width;
  return (width / 6) * (g_f3_multi(left, x, y) +
                        g_f3_multi(right, x, y) +
                        g_f3_multi(mid, x, y) * 4);
}
