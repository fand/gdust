...
void
DataSet::readFile(const char *src) {
...
void
DataSet::writeMultiSamplesDir(const char *dst) {
  ...
  for (int i = 0; i < this->N; i++) {
    snprintf(name, sizeof(name), "%s/timeseries%d", dst, i+1);
...
void
DataSet::perturbateNormal(float mu, float sigma) {
