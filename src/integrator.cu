...
Integrator* Integrator::create (Integrator::Method method) {
  ...
  else if (method == Integrator::Simpson)    return new SimpsonIntegrator();
  else return new MonteCarloIntegrator();
}


void Integrator::prepare_match (TimeSeries &ts, TimeSeriesCollection &tsc)
{
    // Fetch ts/tsc info.
